        ...
        ngx_http_fancyindex_loc_conf_t *alcf)
{
    ...
    for (i = 0; i < entries.nelts; i++) {
        ...
        len += ngx_sizeof_ssz("{url: \"")
            ...
            + ngx_sizeof_ssz("\", size: \"")
            + 20 /* File size */
            + ngx_sizeof_ssz("\", date: \"")    /* Date prefix */
            + ngx_sizeof_ssz("\"},\n") /* Date suffix */
            + 2 /* CR LF */
            ;
    }
    ...
    for (i = 0; i < entries.nelts; i++) {
        b->last = ngx_cpymem_ssz(b->last, "{url: \"");
        ...
        *b->last++ = ',';
        ...
        b->last = ngx_cpymem_ssz(b->last, " link: \"");
        ...
        if (len > alcf->name_length) {
            b->last = ngx_cpymem_ssz(last, "\", size: \""); 
            ...
            b->last = ngx_cpymem_ssz(b->last, "\", size: \"");
        }
        ...
        b->last = ngx_cpymem_ssz(b->last, "\", date: \"");
